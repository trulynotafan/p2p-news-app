...
document.body.innerHTML = `
  ...
  async function render_view (view, ...args) {
    ...
    const renderers = {
      ...
      config: async () => {
        ...
        const active_code = pairing_manager_ref ? pairing_manager_ref.get_verification_code() : null
        if (active_code) {
  ...
  async function handle_create_invite () {
    try {
      ...
      const { invite_code, verification_code, pairing_manager } = result
      pairing_manager_ref = pairing_manager
...
},{"buffer":5,"p2p-news-app":1}],3:[function(require,module,exports){
(function (Buffer){(function (){
