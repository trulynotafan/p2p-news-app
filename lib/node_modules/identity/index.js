...
function identity (config = {}) {
  ...
  let events_drive; let events_store; let ds_manager; let pairing_manager; let swarm; let dht; let keypair
  ...
  const app_audits = new Map() // app_id -> auditcore instance
  ...
  async function authenticate (auth_data) {
    ...
    try {
      if (mode === 'pair' && invite_code) {
        ...
        await init_vault_structures({ store: network_store, vault_bee_key: invite_data.vault_bee_key, vault_audit_key: invite_data.vault_audit_key })
  ...
  async function register_app (app_id, app_config) {
    ...
    if (!network_store) throw new Error('Network store not initialized')
    return audit_wrap('register_app', { app_id }, async function () {
      ...
      const app_audit = create_auditcore({
        store: network_store.namespace(`app-audit-${app_id}`)
      })
      await app_audit.ready()
      app_audits.set(app_id, app_audit)
      // Link app auditcore to vault's root auditcore
      if (vault_audit) {
        const audit_key = b4a.toString(app_audit.key, 'hex')
        await vault_audit.append({ type: 'app_audit_linked', data: { app_id, audit_key } })
      }
  ...
  async function load_app_audit (app_id, audit_key) {
    if (!network_store) throw new Error('Network store not initialized')
    if (app_audits.has(app_id)) return app_audits.get(app_id)
    const app_audit = create_auditcore({
      store: network_store.namespace(`app-audit-${app_id}`),
  ...
  function get_app_audit (app_id) { return app_audits.get(app_id) || null }
...
async function start_networking (options = {}) {
  ...
  if (is_cli) {
    ...
    swarm.on('connection', create_cli_connection_handler(store, name, get_primary_key, mnemonic_data.keypair))
...
***************************************/
function create_cli_connection_handler (store, name, get_primary_key, keypair) {
...
***************************************/
function create_socket_error_handler (resolved_state, relay_url, reject) {
...
***************************************/
function create_socket_open_handler (socket, resolved_state, name, relay_url, topic, store, get_primary_key, get_primary_structure, resolve, reject) {
