  ...
  ***************************************/
  ...
  async function init_blog (options) {
    ...
    if (vault_bee && network_store && swarm_instance) {
      ...
      const app = await identity.get_app()
      ...
      if (app.audit_key) await identity.load_app_audit(app.audit_key)
      /* Writer access management is handled by the vault (identity-operations).
      The datashell wraps the vault, auto-namespacing all calls per app instance,
      so the app never needs to know its own ID. */
