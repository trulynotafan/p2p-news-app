  ...
  ***************************************/
  ...
  async function init_blog (options) {
    ...
    if (vault_bee && network_store && swarm_instance) {
      ...
      /* Now here you can I have moved the watchers stuff to vault
      now I have made it so that the vayult dynamically processes the writers request
      based on a specifc app, still one issue here, the "APP_ID" param, meaning app still knows its not the 
      only app, now we have discussed that datashell will "translate" the app specific calls, 
      so hopefully in the next commit I can remove JUST THE APP_ID parameter from here, rest of the code would remian unchanged.  */ 
      await identity.request_writer_access(APP_ID)
      await identity.wait_for_writer_access(APP_ID)
    ...
    await identity.log_bootstrap_device(APP_ID)
    // Register app in vault
    await register_app_with_vault()
    identity.start_writer_watcher(APP_ID)
    await setup_peer_handlers()
