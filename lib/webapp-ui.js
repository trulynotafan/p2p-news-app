...
const state = {
  store: null,
  username: uservault.username || localStorage.getItem('username') || '',
  current_view: null,
  is_ready: false,
  is_joining: false,
  swarm: null,
...
async function init_blog_app () {
  try {
    ...
    state.default_relay = get_default_relay()
    ...
    if (error_msg.includes('Relay connection failed') || error_msg.includes('Relay connection closed')) {
      ...
      document.getElementById('relay_retry_btn').addEventListener('click', () => location.reload())
    ...
    if (devices_list) {
      ...
      if (devices.length === 0) {
        ...
        for (const device of devices) {
          ...
          let remove_btn = is_my_device ? '' : `<button class="remove-device-btn" style="margin-top: 10px; background: #dc3545; color: white; border: none; padding: 5px 10px; border-radius: 3px; cursor: pointer;">Remove Device</button>`
...
async function handle_reset_all_data () {
  ...
  try {
    ...
    if (typeof requestFileSystem !== 'undefined' || typeof webkitRequestFileSystem !== 'undefined') {
      const request_file_system = typeof requestFileSystem !== 'undefined' ? requestFileSystem : webkitRequestFileSystem
      const PERSISTENT = typeof self !== 'undefined' ? self.PERSISTENT : 1
      await new Promise((resolve, reject) => cleanup_file_system(request_file_system, PERSISTENT, resolve, reject))
    }
    if (state.store) {
      try { await state.store.close() } catch (err) { }
    }
    location.reload()
  } catch (err) {
    alert('Reset error: ' + err.message)
  }
  function cleanup_file_system (request_file_system, PERSISTENT, resolve, reject) {
    request_file_system(PERSISTENT, 1024 * 1024, (fs) => read_filesystem_root(fs, resolve, reject), reject)
  }
  function read_filesystem_root (fs, resolve, reject) {
    fs.root.createReader().readEntries((entries) => process_filesystem_entries(entries, resolve), reject)
  }
  function process_filesystem_entries (entries, resolve) {
    if (!entries.length) return resolve()
    let completed = 0
    const total = entries.length
    function on_entry_removed () {
      completed++
      if (completed === total) resolve()
    }
    function remove_entry (entry) {
      if (entry.isFile) {
        entry.remove(on_entry_removed, on_entry_removed)
      } else {
        entry.removeRecursively(on_entry_removed, on_entry_removed)
      }
    }
