...
// auth handled by setup.js (vault-ui) before this loads

const blog_app = require('p2p-news-app')

// webapp ui fucntion, called by datashell with vault parameter
// User is already registered when this runs (setup.js ran first)
async function webapp_ui(vault, existing_api_result) {
  ...
  async function init_blog_app() {
    try {
      ...
      // basically if the user "pairs" we dont have to reintialize the app
      if (api) {
  ...
  // if usernames exists just intialize the app
  if (username) {
    init_blog_app()
  }
}
