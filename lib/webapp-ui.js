...
document.body.innerHTML = `
  ...
  async function make_network () {
    ...
    try {
      ...
      // Init blog
      const result = await api.init_blog({ username })
      store = result.store
      swarm = result.swarm
  ...
  async function join_network () {
    ...
    try {
      ...
      // Initialize blog with invite code
      ...
      explore: async () => {
        ...
        if (discovered.size > 0) {
          ...
          for (const [key, peer] of discovered) {
            ...
            html += `<div><h5>${escape_html(display_name)}'s Blog (${escape_html(peer.title)})</h5><p><code>${key}</code></p><button class="subscribe-btn" data-key="${key}">Subscribe</button></div><hr>`
          }
        }
  ...
  async function handle_create_invite () {
    try {
      ...
      invite_result.innerHTML = `<p>Invite created! Share this code:</p><input class="invite-code-display" readonly value="${invite_code}" style="width: 400px;"><button class="copy-invite-btn">Copy</button><p><small>Keep this page open while others join.</small></p>`
  ...
  function handle_upload_avatar () {
    ...
    reader.onload = async function (e) {
      try {
        ...
        await api.upload_avatar(imageData, file.name)
  ...
  document.addEventListener('click', (event) => {
    ...
    if (target.classList.contains('copy-address-btn')) navigator.clipboard.writeText(document.querySelector('.blog-address-input').value)
    ...
    if (rawBtnClass) {
      const structureName = rawBtnClass.replace('raw-', '').replace('-btn', '')
      handle_raw_data('show', structureName)
    }
  })
