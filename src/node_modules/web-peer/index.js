...
async function start_browser_peer (options = {}) {
  ...
  socket.addEventListener('open', async () => {
    ...
    await core.ready()
    await core.append('This is some data')
    ...
    swarm.on('connection', (relay, details) => {
      ...
      const handlers = {
        ...
        on_feedkey: async ({key_buffer}) => {
           ...
           const cloned_core = store.get(b4a.from(key_buffer, 'hex'))
           cloned_core.on('append', async () => {

            let flag = false
                const L = cloned_core.length
    if (!flag) {
      flag = true
      for (var i = 0; i < L; i++) {
        const raw = await cloned_core.get(i)
        console.log('download old:', b4a.toString(raw, 'utf-8'))
      }
    }
           console.log("New data available", L)
          const raw = await cloned_core.get(L)
          cconsole.log('Data: ', b4a.toString(raw, 'utf-8'))})
          cloned_core.ready().then(() => {
            console.log('Cloned core ready:', b4a.toString(cloned_core.key, 'hex'))
