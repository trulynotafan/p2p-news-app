...
const { parse_feed_key } = require('../helpers/parsing-helpers/index.js')
const topic = b4a.from('ffb09601562034ee8394ab609322173b641ded168059d256f6a3d959b2dc6021', 'hex')

/******************************************************************************
  START
******************************************************************************/
// function now accepts options object instead of CLI args array
//  so now other modules can easily pass { name: 'my-peer' }
async function start_native_peer (options = {}) {
  // Removed other parsign and validation because that will happen at cli.js
    ...
    const identity_channel = mux.createChannel({
      ...
      onopen: () => {
          ...
          onmessage: async (message) => {
            try {
              ...
              if (message.type === 'protocol') {
                // Will now use standardized messages..
        ...
        protocol_msg.send({
          ...
          data: {
            name: name,
            mode: 'native'  // Now using (type, data) but data will have more properties
          }
        })
        protocol_msg.send({
          type: 'feedkey',
          data: core.key.toString('hex')  // feedkey is now also in 'data' property
        })
      }
    })
...
// No more helper functions cause no cli parsing logic here anymore. 
