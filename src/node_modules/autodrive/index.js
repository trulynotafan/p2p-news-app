...
function create_autodrive(store, bootstrap, opts = {}) {
  ...
  const api = {
    ...
    async put(path, content) {
      await api.ready()
      await state.base.append({ type: 'put', path, content: content.toString('base64') })
    ...
    async list(folder) {
      return state.drive.list(folder)
    },
    ...
    is_writer(key) {
      const key_str = typeof key === 'string' ? key : key.toString('hex')
      if (key_str === state.base.key.toString('hex')) return true
      return state.writers.has(key_str) || (state.base.writers && state.base.writers.some(w => w.key.toString('hex') === key_str))
    },
    async exists(path) {
      try {
    ...
    replicate(stream_or_initiator, opts = {}) {
      if (typeof stream_or_initiator === 'boolean') return state.base.replicate(stream_or_initiator, opts)
      const stream = stream_or_initiator
      return state.base.replicate(stream.isInitiator, { ...opts, stream })
    },
    async sync(other_drive, timeout = 10000) {
      return new Promise((resolve, reject) => {
        const timeout_id = setTimeout(() => reject(new Error('Sync timeout')), timeout)
        const stream1 = api.replicate(true)
        const stream2 = other_drive.replicate(false)
        stream1.pipe(stream2).pipe(stream1)
        stream1.on('end', () => { clearTimeout(timeout_id); resolve() })
        stream1.on('error', (err) => { clearTimeout(timeout_id); reject(err) })
      })
    },
...
module.exports = { create_autodrive, create_wildcard_invite, verify_challenge, invite_encoder }
