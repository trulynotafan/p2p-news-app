...
let profile_drive, events_drive, profile_store, events_store // Identity-related, managed by identity-helper
...
async function sync_events_to_subscriptions () {
  ...
  try {
    const events = await identity_helper.get_events()
...
async function subscribe (key) {
  ...
  try {
    ...
    await identity_helper.log_event('sub', { peer_key: key })
  ...
  for (let i = 1; i < autobase.view.length; i++) { // Skip index 0 (blog-init)
    try {
      ...
      if (validate_blog_post(entry)) {
        ...
        const device_name = data.device ? await identity_helper.get_device_name(data.device) : null
...
// Get raw data from blog structures
async function get_raw_data (type) {
  ...
  if (type === 'posts') {
    const target = drive
    if (!target) return 'No posts autodrive'
    await target.ready()
    const files = []
  ...
  // Delegate to identity helper for profile and events
  return identity_helper.get_raw_data(type)
}
...
function set_drive (type, drive_instance, store_instance) {
  if (type === 'profile') {
    profile_drive = drive_instance
    profile_store = store_instance
    identity_helper.set_profile_drive(drive_instance, store_instance)
  } else if (type === 'events') {
    events_drive = drive_instance
    events_store = store_instance
    identity_helper.set_events_drive(drive_instance, store_instance, setup_events_sync_listeners)
  }
...
// Convenience wrappers
const set_profile_drive = (drive_instance, store_instance) => set_drive('profile', drive_instance, store_instance)
const set_events_drive = (drive_instance, store_instance) => set_drive('events', drive_instance, store_instance)
...
module.exports = {
  ...
  get_profile: (key = null) => {
    const profile_instance = key ? profile_cache.get(key) : profile_drive
    return identity_helper.get_profile(profile_instance)
  },
  get_avatar_content: (key = null) => {
    const profile_instance = key ? profile_cache.get(key) : profile_drive
    return identity_helper.get_avatar_content(profile_instance)
  },
  get_raw_data,
  ...
  get_events: identity_helper.get_events,
  get_paired_devices: identity_helper.get_paired_devices,
  get_device_name: identity_helper.get_device_name,
  remove_device: (device) => identity_helper.remove_device(device, blog_autobase, drive),
...
async function handle_unsubscribe (key) {
  ...
  await identity_helper.log_event('unsub', { peer_key: key })
