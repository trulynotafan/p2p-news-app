...
function set_drive (type, drive, store_instance) {
  ...
  // Setup replication
  if (store.swarm && store_instance) {
    function handle_drive_replication (conn) {
      store_instance.replicate(conn)
    }
    store.swarm.connections.forEach(handle_drive_replication)
  }
