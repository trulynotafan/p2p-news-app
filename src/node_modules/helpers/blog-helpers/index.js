...
async function init_blog (options) {
  ...
  // For profile drive, always create as new drive initially
  // It will be set up properly when init data replicates (same as existing drive pattern)
...
function setup_event_handlers () {
  ...
  async function handle_autobase_update () {
    // Check if we need to set up profile drive when autobase gets updated
    if (blog_autobase.view.length > 0) {
      try {
        const init_raw_data = await blog_autobase.view.get(0)
        const init_entry = JSON.parse(b4a.toString(init_raw_data))
        if (validate_blog_init(init_entry) && init_entry.data.profile_drive_key) {
          const profile_drive_key = init_entry.data.profile_drive_key
          const current_key = profile_drive.base.key ? b4a.toString(profile_drive.base.key, 'hex') : null
          
          // Only recreate if we don't have the right key
          if (current_key !== profile_drive_key) {
            // Use separate namespaced store like existing drive pattern
            const joined_profile_store = store.namespace(`joined-profile-${profile_drive_key.slice(0, 16)}`)
            profile_drive = create_autodrive({ store: joined_profile_store, bootstrap: b4a.from(profile_drive_key, 'hex') })
            await profile_drive.ready()
          }
        }
      } catch (err) {
        console.warn('Error checking profile drive setup:', err)
      }
    }
    emitter.emit('update')
  }
...
async function get_avatar_content (key = null) {
  ...
  try {
    await profile_instance.ready()
    const avatar_data = await profile_instance.get('/avatar.svg')
