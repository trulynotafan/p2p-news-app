const { start: start_browser_peer } = require('./web-peer-v2')
...
async function join_network () {
  ...
  try {
    ...
    // init_blog already called in start_browser_peer, pairing is complete
    blog_helper.on_update(() => { if (current_view) render_view(current_view) })
    
    // Get username from pairing result (now available!)
    const pairing_result = blog_helper.get_pairing_result()
    ...
    if (pairing_result?.username) {
      ...
      console.warn('[page] No username in pairing result, using default')
      username = 'User'
    }
      ...
      view_el.innerHTML = `
        ...
        <div>
          ...
          <div class="raw-data-options" style="display: none; margin-top: 10px;">
            ${await generate_raw_data_buttons()}
          </div>
      ...
      if (view === 'config') {
        ...
        if (devices.length === 0) {
          ...
          for (const device of devices) {
            ...
            // Build remove button data attributes dynamically
            ...
            // Build keys display dynamically
            let keys_html = ''
            for (const [key, value] of Object.entries(device)) {
              if (key.endsWith('_writer')) {
...
async function handle_remove_device (button) {
  // Build device object dynamically from all data attributes
  const device = {}
  for (const [key, value] of Object.entries(button.dataset)) {
    // Convert camelCase back to snake_case (e.g., metadataWriter -> metadata_writer)
...
// Generate raw data buttons dynamically
async function generate_raw_data_buttons () {
  if (!blog_helper.get_structure_names) {
    // Fallback for old blog-helpers
    return `
      <button class="raw-metadata-btn">Metadata</button>
      <button class="raw-posts-btn">Drive</button>
      <button class="raw-profile-btn">Profile</button>
      <button class="raw-events-btn">Events</button>
    `
  }
  
  const structure_names = blog_helper.get_structure_names()
  let buttons_html = ''
...
function handle_document_click (event) {
  ...
  // Dynamic raw data button handling
