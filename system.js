...
***************************************/
        async function handle_seed () {
          ...
          try {
            ...
            vault.session_set('username', username)
            vault.session_set('auth_mode', 'seed')
            vault.session_set('joined_app', 'p2p-news-app')
            // Show system bar
            show_system_bar()
...
***************************************/
        async function handle_pair () {
          ...
          try {
            ...
            vault.session_set('pending_invite_code', invite_code)
            vault.session_set('auth_mode', 'pair')
            ...
            async function handle_vault_ready (auth_data) {
              ...
              if (auth_data.username && auth_data.username !== 'pairing-user') {
                vault.session_set('username', auth_data.username)
              }
...
***************************************/
        async function handle_join_app (app_id) {
          ...
          vault.session_set('joined_app', app_id)
...
***************************************/
        const existing_username = vault.session_get('username')
        const auth_mode = vault.session_get('auth_mode') || 'seed'
        const joined_app = vault.session_get('joined_app')
...
***************************************/
      ...
      ' <button class="launch-btn" data-id="' + app.id + '">Launch</button></div>'
          ...
          picker.addEventListener('click', function (e) {
            ...
            vault.session_set('joined_app', e.target.dataset.id)
            vault.complete_authentication()
            picker.remove()
          })
        }
...
***************************************/
  ...
  ***************************************/
          ...
          const username = vault.session_get('username')
  ...
  ***************************************/
          async function handle_create_invite () {
            ...
            el.innerHTML = '<input readonly value="' + result.invite_code + '" style="width:300px;font-family:monospace"> <button class="copy-btn">Copy</button>'
            ...
            await vault.setup_vault_pairing({
              ...
              username: vault.session_get('username') || 'Unknown',
